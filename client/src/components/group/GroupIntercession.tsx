        ...
        {activeTab === 'feed' ? (
          <motion.div key="feed" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
              ...
              {!audioUrl ? (
                <div className="flex flex-col items-center py-10 bg-zinc-50 rounded-[30px] border border-dashed border-zinc-200">
                  ...
                  <button 
                    ...
                    className={`w-20 h-20 rounded-full flex items-center justify-center shadow-2xl active:scale-95 transition-all ${isRecording ? 'bg-red-500 shadow-red-200' : 'bg-[#4A6741] shadow-[#4A6741]/20'}`}
                  >
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      { id: 'targets', label: '대상자만' },
                      { id: 'private', label: '나만보기' }
                    ].map(opt => (
                      <button 
                        key={opt.id} 
                        onClick={() => setVisibility(opt.id as any)} 
                        className={`py-3 rounded-2xl flex flex-col items-center gap-1.5 border transition-all ${visibility === opt.id ? 'bg-[#4A6741] text-white' : 'bg-zinc-50 text-zinc-400'}`}
                      >
            ...
            <div className="space-y-4">
              ...
              {[1, 2].map((i) => {
                const isAnswered = i === 2; // 빌드 오류 수정됨
                return (
                  <div key={i} className={`bg-white rounded-[32px] p-6 shadow-sm border ${isAnswered ? 'bg-[#4A6741]/5 border-[#4A6741]/20' : 'border-zinc-50'}`}>
                    <div className="flex justify-between items-start mb-5 text-left">
                      <div className="flex gap-3 text-left">
                        <div className="w-11 h-11 bg-zinc-100 rounded-2xl flex items-center justify-center font-black text-zinc-400 text-xs">
                          {i === 1 ? "박" : "김"}
                        </div>
                        <div className="text-left">
                          <div className="flex items-center gap-2">
                            ...
                            {isAnswered && (
                              <span className="bg-[#4A6741] text-white text-[8px] font-black px-1.5 py-0.5 rounded-md flex items-center gap-0.5 tracking-tighter">
                                <CheckCircle2 size={8}/> Answered
                              </span>
                            )}
                          </div>
                          <div className="text-[10px] font-bold text-zinc-400">{i}5분 전</div>
                        </div>
                      </div>
                      <MoreVertical size={18} className="text-zinc-300 cursor-pointer"/>
                    </div>
                    ...
                    <div className="bg-zinc-50 rounded-[22px] p-4 flex items-center gap-4 mb-5 border border-zinc-100/50">
                      ...
                      <div className="flex-1 text-left">
                         <div className="text-[12px] font-bold text-zinc-700 truncate">{i === 1 ? "치유를 위한 간절한 기도..." : "새로운 터전에서의 적응"}</div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <motion.button 
                          whileTap={{ scale: 0.9 }}
                          className={`flex items-center gap-1.5 px-3 py-2 rounded-full text-[11px] font-black ${i === 1 ? "bg-rose-50 text-rose-500" : "bg-zinc-50 text-zinc-400"}`}
                        >
              ...
              <div className="relative z-10 space-y-8">
                <div className="flex justify-between items-start">
                  <div className="text-left">
                    ...
                    <h3 className="text-2xl font-black mt-1">{reportData.month}월 소그룹 리포트</h3>
                  </div>
                  <Sparkles size={24} className="text-amber-300" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-black/10 rounded-3xl p-5 text-left border border-white/5">
                    <TrendingUp className="text-amber-300 mb-2" size={20} />
                    <div className="text-2xl font-black">{reportData.totalActivity}</div>
                    <div className="text-[10px] font-bold opacity-60 uppercase tracking-tighter">Total Activity</div>
                  </div>
                  <div className="bg-black/10 rounded-3xl p-5 text-left border border-white/5">
                    <Users className="text-blue-300 mb-2" size={20} />
                    <div className="text-2xl font-black">{reportData.activeMembers}</div>
                    <div className="text-[10px] font-bold opacity-60 uppercase tracking-tighter">Members</div>
                  </div>
                </div>
                <div className="space-y-4">
                    ...
                    {reportData.topKeywords.map((k) => (
                      <div key={k.name} className="space-y-1.5">
                        ...
                        <div className="h-1 bg-white/10 rounded-full overflow-hidden">
                          <motion.div initial={{ width: 0 }} animate={{ width: `${k.value}%` }} className="h-full bg-white" />
                        </div>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="bg-white/10 rounded-3xl p-6 italic border-l-4 border-white/30 text-left">
                  <Quote size={16} className="mb-2 opacity-50 text-white" />
                  <p className="text-sm font-medium leading-relaxed">"우리의 기도가 쌓여 하나님의 일하심이 됩니다."</p>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 px-1">
              <button className="flex items-center justify-center gap-2 bg-zinc-900 text-white py-4 rounded-[24px] text-xs font-black shadow-lg shadow-zinc-200"><Download size={16} /> 이미지 저장</button>
              <button className="flex items-center justify-center gap-2 bg-[#4A6741] text-white py-4 rounded-[24px] text-xs font-black shadow-lg shadow-[#4A6741]/20"><Share2 size={16} /> 리포트 공유</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
