...
} from "lucide-react"; // ✅ 오타 수정
import { motion, AnimatePresence } from "framer-motion";
import { supabase } from "../../lib/supabase"; // ✅ 누락된 임포트 추가
...
export default function GroupIntercession({ groupId, role }: { groupId: string, role: string }) {
  ...
  const timerRef = useRef<any>(null); // ✅ 타입 수정
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 pb-20 text-left">
        ...
        {!audioUrl ? (
          <div className="relative flex flex-col items-center py-10 bg-zinc-50 rounded-[30px] border border-dashed border-zinc-200 overflow-hidden">
            ...
            <p className="mt-6 text-zinc-400 text-[11px] font-black tracking-tight">
              {isRecording ? "녹음 중 - 중단하려면 버튼 클릭" : "버튼을 눌러 기도를 시작하세요"}
            </p>
          </div>
        ) : (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6">
            <div className="bg-[#4A6741] rounded-[24px] p-5 flex items-center gap-4 shadow-lg shadow-[#4A6741]/20 text-white">
              <button className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-md"><Play size={20} fill="white"/></button>
              <div className="flex-1"><div className="h-1 bg-white/20 rounded-full overflow-hidden"><div className="w-1/3 h-full bg-white" /></div></div>
              <button onClick={() => setAudioUrl(null)} className="w-8 h-8 flex items-center justify-center bg-black/10 rounded-full"><X size={16}/></button>
            </div>
            
            <button onClick={() => setShowMemberSelector(true)} className="flex items-center gap-2 px-4 py-2 bg-zinc-100 rounded-full text-[11px] font-black text-zinc-600 w-fit">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <button key={opt.id} onClick={() => setVisibility(opt.id as any)} className={`py-3 rounded-2xl flex flex-col items-center gap-1.5 transition-all border ${visibility === opt.id ? 'bg-[#4A6741] border-[#4A6741] text-white shadow-md' : 'bg-zinc-50 border-zinc-100 text-zinc-400'}`}>
      ...
      <div className="space-y-4">
        ...
        {[1, 2].map((i) => (
          <motion.div key={i} whileHover={{ y: -2 }} className="group bg-white rounded-[32px] p-6 shadow-sm border border-zinc-50 hover:shadow-xl hover:shadow-zinc-200/50 transition-all">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <button className="flex items-center gap-1.5 px-3 py-2 bg-pink-50 text-pink-500 rounded-full text-[11px] font-black active:scale-90 transition-all"><Heart size={14} fill={i === 1 ? "currentColor" : "none"}/> 24</button>
                <button className="flex items-center gap-1.5 px-3 py-2 bg-zinc-50 text-zinc-400 rounded-full text-[11px] font-black active:scale-90 transition-all"><MessageSquare size={14}/> 8</button>
              </div>
              <button className="text-[11px] font-black text-[#4A6741] flex items-center gap-1 bg-[#4A6741]/5 px-3 py-2 rounded-full">기도에 동참하기 <ChevronRight size={12}/></button>
            </div>
          </motion.div>
        ))}
      </div>
