...
export default function GroupIntercession({ groupId, role }: { groupId: string, role: string }) {
  ...
  const [showMemberSelector, setShowMemberSelector] = useState(false); // ✅ 원본 복구
  const [visibility, setVisibility] = useState<'public' | 'targets' | 'private'>('public'); // ✅ 원본 복구
  const [waves, setWaves] = useState(Array(15).fill(20)); // ✅ 원본 복구
  ...
  const reportData = {
    ...
    totalActivity: 254,
    activeMembers: 12,
    topKeywords: [{ name: "위로", value: 85 }, { name: "비전", value: 60 }, { name: "가정", value: 45 }]
  };

  // ✅ 음성 파동 및 타이머 로직 (원본 100% 복구)
  useEffect(() => {
    if (isRecording) {
        ...
        {activeTab === 'feed' ? (
          <motion.div key="feed" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
              ...
              {!audioUrl ? (
                <div className="flex flex-col items-center py-10 bg-zinc-50 rounded-[30px] border border-dashed border-zinc-200">
                  ...
                  <button 
                    ...
                    className={`w-20 h-20 rounded-full flex items-center justify-center shadow-2xl active:scale-95 transition-all ${isRecording ? 'bg-red-500' : 'bg-[#4A6741]'}`}
                  >
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {['public', 'targets', 'private'].map(id => (
                      <button 
                        ...
                        className={`py-3 rounded-2xl flex flex-col items-center gap-1.5 border transition-all ${visibility === id ? 'bg-[#4A6741] text-white' : 'bg-zinc-50 text-zinc-400'}`}
                      >
            ...
            <div className="space-y-4">
              ...
              {[1, 2].map((i) => {
                const isAnswered = i === 2; // 빌드 안전 변수 선언
                return (
                      ...
                      <div className="flex gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-black text-zinc-800">박마리아 집사</span>
                            {isAnswered && (
          ...
          <motion.div key="report" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
            <div className="bg-[#4A6741] rounded-[40px] p-8 text-white shadow-2xl relative overflow-hidden text-left">
              <div className="relative z-10 space-y-8">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-black/10 rounded-3xl p-5 border border-white/5">
                    ...
                    <div className="text-[10px] font-bold opacity-60 uppercase tracking-tighter">Activity</div>
                  </div>
                  <div className="bg-black/10 rounded-3xl p-5 border border-white/5">
