...
export default function ReadingPage() {
  ...
  useEffect(() => {
    async function fetchBooks() {
      // bible_books 테이블에서 전체 목록을 가져옵니다.
  ...
  // [수정] 탭 변경 및 단계 이동 로직
  const handleTabClick = (tab: 'OLD' | 'NEW') => {
    setActiveTab(tab);
    setProgressStep('BOOKS');
  };

  const handleBookClick = (book: any) => {
    setSelectedBook(book);
    setProgressStep('CHAPTERS');
  };
        ...
        {showProgressModal && (
          <div className="fixed inset-0 z-[300] bg-black/60 backdrop-blur-sm flex items-end justify-center">
            <motion.div 
              ...
              className="bg-white w-full max-w-md rounded-t-[40px] max-h-[85vh] flex flex-col shadow-2xl"
            >
              {/* 헤더 영역 */}
              <div className="p-6 border-b flex justify-between items-center bg-white sticky top-0 z-10 rounded-t-[40px]">
                <div className="flex items-center gap-2">
                  {progressStep !== 'TAB' && (
                    <button 
                      onClick={() => setProgressStep(progressStep === 'CHAPTERS' ? 'BOOKS' : 'TAB')} 
                      className="p-2 -ml-2 text-zinc-400 hover:text-zinc-600 active:scale-90 transition-transform"
                    >
                      <ChevronLeft size={24} />
                    </button>
                  )}
                  <h3 className="text-xl font-black">
                    {progressStep === 'TAB' ? "상세 진척도" : 
                     progressStep === 'BOOKS' ? (activeTab === 'OLD' ? "구약 성경" : "신약 성경") : 
                     selectedBook?.book_name}
                  </h3>
                </div>
                <Button variant="ghost" size="icon" onClick={() => { setShowProgressModal(false); setProgressStep('TAB'); }}>
                  <X size={24} />
                </Button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-5">
                ...
                {progressStep === 'TAB' && (
                  <div className="space-y-4">
                    {[
                      ...
                      <button 
                        key={tab.id}
                        onClick={() => handleTabClick(tab.id as 'OLD' | 'NEW')}
                        className="w-full p-6 bg-zinc-50 rounded-[28px] text-left border border-zinc-100 active:scale-[0.98] transition-all"
                      >
                        ...
                        <div className="h-3 bg-zinc-200 rounded-full overflow-hidden">
                          <motion.div initial={{ width: 0 }} animate={{ width: `${tab.percent}%` }} className="h-full bg-[#5D7BAF]" />
                        </div>
                      </button>
                    ))}
                  </div>
                ...
                {progressStep === 'BOOKS' && (
                  <div className="grid grid-cols-1 gap-3 pb-10">
                    {books
                      .filter(b => activeTab === 'OLD' ? b.book_id <= 39 : b.book_id > 39)
                      .map(b => (
                        <button 
                          key={b.book_id} 
                          onClick={() => handleBookClick(b)}
                          className="w-full p-4 bg-zinc-50 rounded-2xl flex items-center justify-between border border-transparent active:bg-zinc-100 active:scale-[0.98] transition-all"
                        >
                          <div className="text-left">
                            <p className="font-bold text-zinc-800 text-sm">{b.book_name}</p>
                            <p className="text-[10px] font-bold text-[#5D7BAF] mt-0.5">진행중</p>
                          </div>
                ...
                {progressStep === 'CHAPTERS' && (
                  <div className="grid grid-cols-5 gap-3 p-1 pb-10">
                    {[...Array(selectedBook?.total_chapters || 0)].map((_, i) => (
                      <div 
                        key={i} 
                        className={`aspect-square rounded-2xl flex flex-col items-center justify-center font-bold text-xs transition-all shadow-sm ${
                          i < 12 ? 'bg-blue-50 text-[#5D7BAF] border-2 border-blue-100' : 'bg-zinc-50 text-zinc-300 border border-zinc-100'
                        }`}
                      >
                        {i < 12 ? (
                          <>
                            <CheckCircle2 size={16} className="mb-0.5" />
                            <span className="text-[8px]">완료</span>
                          </>
                        ) : (
                          <span>{i + 1}</span>
                        )}
