...
export default function ReadingPage() {
  ...
  return (
    <div className="flex flex-col items-center w-full min-h-full bg-[#F8F8F8] overflow-y-auto overflow-x-hidden pt-24 pb-4 px-4">
      ...
      {/* 말씀 카드 영역: AnimatePresence mode="wait" 삭제 (부드러운 속도의 핵심) */}
      <div className="relative w-full flex-1 flex items-center justify-center py-4 overflow-visible" style={{ perspective: "1200px" }}>
        <AnimatePresence initial={false}>
          <motion.div 
            ...
            onDragEnd={onDragEnd}
            style={{ position: "absolute" }} // mode="wait"이 없을 때 겹침 방지
          ...
          >
            <div className="flex-1 overflow-y-auto pr-1 text-center scrollbar-hide font-medium text-zinc-800" style={{ fontSize: `${fontSize}px`, lineHeight: 1.8 }}>
              {loading ? "로딩 중..." : bibleContent.map((v, i) => <p key={i} className="mb-5">{v.content}</p>)}
            </div>
      ...
      <div className="flex items-center gap-8 mt-6 mb-12"> 
        <button onClick={() => setShowAudioControl(true)} className="flex flex-col items-center gap-1.5 text-zinc-400">
          <Headphones size={22} /><span className="font-medium text-[11px]">음성 재생</span>
        </button>
        <button onClick={() => alert("복사되었습니다.")} className="flex flex-col items-center gap-1.5 text-zinc-400">
          <Copy size={22} /><span className="font-medium text-[11px]">복사</span>
        </button>
        <button className="flex flex-col items-center gap-1.5 text-zinc-400">
          <Bookmark size={22} /><span className="font-medium text-[11px]">기록함</span>
        </button>
        <button className="flex flex-col items-center gap-1.5 text-zinc-400">
          <Share2 size={22} /><span className="font-medium text-[11px]">공유</span>
        </button>
      </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <button onClick={() => setIsPlaying(!isPlaying)} className="w-8 h-8 flex items-center justify-center bg-white/20 rounded-full">{isPlaying ? <Pause fill=\"white\" size={14} /> : <Play fill=\"white\" size={14} />}</button>
