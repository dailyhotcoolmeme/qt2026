...
import { resolveApiUrl, isNativeApp } from "../lib/appUrl";

export default function TermsPage() {
  const [match, params] = useRoute("/terms/:type");
  ...
  useEffect(() => {
    addDebug(`match=${match} type=${params?.type ?? "null"}`);
    addDebug(`hash=${window.location.hash}`);
    addDebug(`native=${isNativeApp()}`);

    if (!params?.type) {
      addDebug("params.type 없음 → 로딩 유지");
      return;
    }

    const url = resolveApiUrl(`/api/terms?type=${params.type}`);
    addDebug(`fetch → ${url}`);
    setLoading(true);
    setError(false);
    ...
    fetch(url)
      .then(async (res) => {
        ...
        addDebug(`title=${data?.title ?? "없음"}`);
        if (!data?.title) { setError(true); }
        else { setTerm(data); }
        setLoading(false);
      })
      .catch((e) => { addDebug(`ERROR: ${e}`); setError(true); setLoading(false); });
  }, [params?.type]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F8F8F8", display: "flex", flexDirection: "column" }}>
      ...
      {/* 디버그 오버레이 */}
      {debugInfo.length > 0 && (
        <div onClick={() => setDebugInfo([])} style={{
          position: "fixed", top: 60, left: 0, right: 0, zIndex: 9999,
          background: "rgba(0,0,0,0.85)", padding: "12px 16px",
          maxHeight: "50vh", overflowY: "auto",
        }}>
          <p style={{ color: "#4ade80", fontSize: 12, fontWeight: 700, marginBottom: 6 }}>
            🔍 TermsPage 디버그 (탭하면 닫힘)
          </p>
          {debugInfo.map((line, i) => (
            <p key={i} style={{ color: "#e4e4e7", fontSize: 11, fontFamily: "monospace", margin: 0, lineHeight: 1.6 }}>{line}</p>
          ))}
        </div>
      )}
