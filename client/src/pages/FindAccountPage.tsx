...
import { ArrowLeft, Mail, User, Send, CheckCircle2, AlertCircle } from "lucide-react";
...
export default function FindAccountPage() {
  ...
  const handleResetPw = async () => {
    ...
    try {
      ...
      setResult({ success: false, message: e.message || "이메일 발송에 실패했습니다." });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen w-full bg-[#F8F8F8] flex flex-col">
      ...
      <div className="px-6 pt-12 pb-6 flex items-center gap-4">
        <button onClick={() => setLocation("/auth")} className="p-2 -ml-2 text-zinc-400 hover:text-zinc-900 transition-colors">
          <ArrowLeft size={24} />
        </button>
      ...
      <div className="flex-1 px-8 pt-4">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-[24px] p-5 shadow-sm border-2 border-transparent focus-within:border-[#4A6741] transition-all flex items-center gap-4">
          ...
          <button 
            ...
            className="w-full h-[64px] bg-[#4A6741] disabled:bg-zinc-200 text-white rounded-[22px] font-black shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-all"
          >
            {isLoading ? <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1 }}><Send size={20}/></motion.div> : "확인하기"}
          </button>
        </div>
