...
export default function RegisterPage() {
  ...
  const checkDuplicate = async (field: "username" | "nickname" | "email") => {
    ...
    if (field === "email") {
      ...
      if (!emailId || !domain) return setErrorModal({ show: true, msg: "이메일 아이디와 도메인을 모두 입력해주세요." });
    ...
    try {
      ...
      if (field === "username") {
        ...
        if (data) { setEmailStatus('error'); setEmailMsg("이미 가입된 정보입니다 😢"); }
        else { setEmailStatus('success'); setEmailMsg("사용 가능한 정보입니다!"); }
      }
  ...
  const onSubmit = async (values: any) => {
    ...
    if (emailStatus !== 'success') return setErrorModal({ show: true, msg: "이메일 중복 확인을 완료해 주세요." });
    if (!isPasswordMatch) return setErrorModal({ show: true, msg: "비밀번호가 일치하지 않거나 8자 미만입니다." });
    ...
    try {
      ...
      // Auth 가입 (Metadata를 함께 넘겨 트리거 오류 방지 시도)
        ...
        {errorModal.show && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center px-8 bg-black/40 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              ...
              <h3 className="font-black text-zinc-900 mb-2" style={{ fontSize: `${fontSize * 1.1}px` }}>알림</h3>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-6">
        ...
        <div className={`rounded-3xl p-5 border-2 shadow-sm mb-4 transition-all duration-300 ${getBorderStyle(usernameStatus)}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="button" onClick={() => checkDuplicate("username")} className="font-bold px-4 py-2 rounded-xl bg-zinc-900 text-white shrink-0 active:scale-95 transition-transform shadow-sm" style={{ fontSize: `${fontSize * 0.7}px` }}>중복확인</button>
          </div>
        </div>
        ...
        <div className={`rounded-3xl p-5 border-2 shadow-sm mb-4 transition-all duration-300 ${getBorderStyle(emailStatus)}`}>
          ...
          <div className="flex flex-col gap-3">
            ...
            <button type="button" onClick={() => checkDuplicate("email")} className="w-full font-bold py-3 rounded-xl bg-zinc-900 text-white active:scale-95 transition-transform shadow-md" style={{ fontSize: `${fontSize * 0.8}px` }}>이메일 중복확인</button>
          </div>
        </div>

        {/* 비밀번호 */}
        <div className={`rounded-3xl p-5 border-2 shadow-sm mb-4 transition-all duration-300 ${isPasswordMatch ? 'border-[#4A6741] bg-[#4A6741]/5' : 'border-[#4A6741]/10'}`}>
          <div className="flex justify-between items-center mb-2">
        ...
        {/* 깨지지 않는 선택 입력 레이아웃 */}
        <div className="bg-white rounded-3xl border-2 border-[#4A6741]/5 divide-y divide-zinc-50 overflow-hidden shadow-sm">
             <div className="flex items-center justify-between p-5 min-h-[64px]">
                ...
                <input {...register("fullName")} placeholder="실명 입력" className="bg-transparent text-right outline-none text-zinc-800 font-medium flex-1 ml-4" style={{ fontSize: `${fontSize}px` }} />
             </div>
             <div className="flex items-center justify-between p-5 min-h-[64px]">
                <span className="font-bold text-zinc-400 shrink-0" style={{ fontSize: `${fontSize * 0.75}px` }}>전화번호</span>
                <input {...register("phone")} placeholder="010-0000-0000" className="bg-transparent text-right outline-none text-zinc-800 font-medium flex-1 ml-4" style={{ fontSize: `${fontSize}px` }} 
                  onChange={(e) => {
             ...
             <div className="flex items-center justify-between p-5 min-h-[64px]">
                ...
                <div className="flex-1 text-right ml-4">
                  {showCustomRank ? (
                    ...
                    <select {...register("rank")} onChange={(e) => e.target.value === "직접 입력" && setShowCustomRank(true)} className="bg-transparent outline-none text-right text-zinc-800 font-medium appearance-none w-full" style={{ fontSize: `${fontSize}px` }}>
             ...
             <div className="flex items-center justify-between p-5 min-h-[64px]">
                ...
                <input {...register("church")} placeholder="교회 이름 입력" className="bg-transparent text-right outline-none text-zinc-800 font-medium flex-1 ml-4" style={{ fontSize: `${fontSize}px` }} />
             </div>
        </div>
