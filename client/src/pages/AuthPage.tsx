...
export default function AuthPage() {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-between bg-[#F8F8F8] px-8 pt-12 pb-24 overflow-hidden relative">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full text-center mt-4">
        ...
        <p className="text-zinc-400 mt-5 font-medium leading-relaxed" style={{ fontSize: `${fontSize * 0.95}px` }}>
          매일의 묵상(QT)과 중보를<br />
          음성으로 기록하고 보관하세요.
        </p>
      </motion.div>
      ...
      <div className="w-full max-w-sm">
        <button onClick={handleKakaoLogin} className="w-full h-[64px] bg-[#FEE500] text-[#3C1E1E] font-bold rounded-[22px] shadow-lg flex items-center justify-center gap-3 mb-4">
        ...
        {isLoginOpen && (
          <>
            ...
            <motion.div 
              ...
              className="fixed bottom-0 left-0 right-0 bg-white rounded-t-[32px] z-[100] px-6 pt-10 pb-12 shadow-2xl"
            >
              ...
              <div className="space-y-3 px-2">
                {/* 아이디 입력 박스 (배경색 통일) */}
                <div className="bg-[#F8F8F8] rounded-[20px] p-4 border-2 border-transparent focus-within:border-[#4A6741]">
                ...
                <div className="bg-[#F8F8F8] rounded-[20px] p-4 border-2 border-transparent focus-within:border-[#4A6741]">
                  ...
                  <div className="flex items-center relative">
                    ...
                    <button type="button" onClick={() => setShowPw(!showPw)} className="text-zinc-300 absolute right-0 outline-none">
                ...
                {errorMsg && <div className="text-red-500 font-bold text-[11px] px-2 pb-2 flex items-center gap-1"><AlertCircle size={14}/> {errorMsg}</div>}

                <button 
                  disabled={isLoading} onClick={handleManualLogin}
                  className="w-full h-[64px] bg-[#4A6741] text-white rounded-[22px] font-black shadow-lg flex items-center justify-center active:scale-95 transition-all mt-2"
                >
